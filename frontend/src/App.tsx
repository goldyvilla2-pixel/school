...
const APPS = [
  ...
  { id: 'courses', name: 'COURSES', icon: 'book', color: '#f59e0b', addText: 'CREATE NEW COURSE' },
            ...
            <div style={{ background: 'white', borderRadius: '35px', border: '2px solid #f1f5f9', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '40px' }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap:'20px' }}>
                        {dynamicFields.filter(f => f.module === mod && f.formType === activeT).map(f => (
                            <div key={f.id} style={{ padding: '25px', background: 'white', borderRadius: '22px', border: '1.5px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div><p style={{ fontWeight: 950 }}>{f.label}</p><p style={{ fontSize: '9px', fontWeight: 950, color: '#875A7B' }}>{f.type} {f.required ? '| REQUIRED' : ''}</p></div>
...
const ModuleRegistry: React.FC<{ module: string }> = ({ module }) => {
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '25px' }}>
                {modFields.map(f => (
                    <div key={f.id}>
                        ...
                        {f.type === 'TEXT' && <input data-required={f.required} defaultValue={sel?.customData[f.id]} id={`f-${f.id}`} style={{ width: '100%', padding: '14px', borderRadius: '14px', border: '1.5px solid #ddd', fontWeight: 900 }} />}
                        {f.type === 'NUMBER' && <input data-required={f.required} type="number" defaultValue={sel?.customData[f.id]} id={`f-${f.id}`} style={{ width: '100%', padding: '14px', borderRadius: '14px', border: '1.5px solid #ddd', fontWeight: 900 }} />}
                        {f.type === 'DROPDOWN' && (<select data-required={f.required} id={`f-${f.id}`} defaultValue={sel?.customData[f.id]} style={{ width: '100%', padding: '14px', borderRadius: '14px', border: '1.5px solid #ddd', fontWeight: 900 }}><option value="">-- SELECT --</option>{globalLists.find(l => l.id === f.globalListId)?.items.map(o => <option key={o} value={o}>{o}</option>)}</select>)}
                        {f.type === 'DATE' && <input data-required={f.required} type="date" defaultValue={sel?.customData[f.id]} id={`f-${f.id}`} style={{ width: '100%', padding: '14px', borderRadius: '14px', border: '1.5px solid #ddd', fontWeight: 900 }} />}
            ...
            <div style={{ mt:'60px', display:'flex', gap:'20px', marginTop:'40px' }}>
                <button onClick={() => {
                    ...
                    modFields.forEach(f => {
                        ...
                        if(f.required && !val) { valid = false; el.style.borderColor = 'red'; } else if (el) { el.style.borderColor = '#ddd'; }
                        c[f.id] = val;
                    });
                    if(!valid) { alert('PLEASE FILL MANDATORY FIELDS'); return; }
                ...
                }} className="odoo-btn btn-primary" style={{ padding: '18px 60px' }}>SAVE</button><button onClick={() => setView('GRID')} className="odoo-btn btn-secondary">CANCEL</button>
            </div>
        </div>
    );
    ...
    if (view === 'PROFILE' && sel) return (
        <div style={{ padding: '40px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', mb: '40px', marginBottom:'40px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '40px' }}>
                <div style={{ background: '#111827', borderRadius: '32px', padding: '30px', color: 'white', height: 'fit-content' }}>
                    <h3 style={{ fontWeight: 950, mb:'30px', marginBottom:'30px' }}>NAVIGATION</h3>
                    <div style={{ display:'flex', flexDirection:'column', gap:'15px' }}>
                        {FORM_TYPES.map(t => (
                            <button key={t} onClick={() => setActiveT(t)} style={{ textAlign:'left', padding:'15px 20px', borderRadius:'15px', border:'none', background: activeT === t ? '#875A7B' : 'transparent', color: 'white', fontWeight: 950, cursor: 'pointer' }}>{t}</button>
                        ))}
                    </div>
                </div>
                <div style={{ background: 'white', borderRadius: '32px', padding: '50px', border: '1.5px solid #f1f5f9' }}>
                    <h3 style={{ fontWeight: 950, mb: '40px', borderBottom: '2px solid #f8fafc', pb: '20px', paddingBottom:'20px', marginBottom: '30px' }}>{activeT} DETAILS</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '40px' }}>
                        {modFields.filter(f => f.formType === activeT).map(f => (
                            <div key={f.id}><label style={{ fontSize: '10px', fontWeight: 950, color: '#94a3b8' }}>{f.label}</label><p style={{ fontWeight: 900 }}>{sel.customData?.[f.id]?.toString() || '—'}</p></div>
                        ))}
                    </div>
                </div>
            </div>
    ...
    return (
        <div style={{ padding: '40px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', mb:'50px', marginBottom:'45px', alignItems:'center' }}>
                <h1 style={{ fontWeight: 950 }}>{module.toUpperCase()} DIRECTORY</h1>
                <div style={{ display:'flex', gap:'20px' }}>
                    <div style={{ position:'relative' }}><Icon name="search" size={18} style={{ position:'absolute', left:'15px', top:'50%', transform:'translateY(-50%)', color:'#94a3b8' }} /><input placeholder="SEARCH RECORDS..." onChange={e => setSearch(e.target.value)} style={{ padding:'14px 20px 14px 45px', borderRadius:'15px', border:'2.5px solid #f1f5f9', fontWeight:950, minWidth:'350px' }} /></div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '30px' }}>
                {filtered.map(r => (
                    <div key={r.id} onClick={() => { setSel(r); setView('PROFILE'); }} style={{ padding: '40px', background: 'white', border: '1.5px solid #f1f5f9', borderRadius: '35px', cursor: 'pointer' }}>
                        <h4 style={{ fontWeight: 950, color: '#875A7B', fontSize:'18px' }}>{r.customData[modFields[0]?.id] || 'NO DATA'}</h4>
                    </div>
                ))}
            </div>
        </div>
    ...
    const render = () => {
        if (!a || a === 'dashboard') return (
            <div style={{ padding: '100px 20px', maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '40px' }}>{APPS.map(app => (<div key={app.id} onClick={() => setA(app.id)} style={{ padding: '45px 25px', borderRadius: '45px', background: 'white', cursor: 'pointer', border: '2px solid #f1f5f9', textAlign: 'center' }}><div style={{ width: '100px', height: '100px', borderRadius: '32px', background: app.color, margin: '0 auto 25px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}><Icon name={app.icon} size={48}/></div><span style={{ fontWeight: 950 }}>{app.name}</span></div>))}</div>
            </div>
        );
...
export const App: React.FC = () => {
    ...
    const [r, setR] = useState<GenericRecord[]>(() => s('erp_rec_v14', []));
    const [d, setD] = useState<DynamicField[]>(() => s('erp_df_v14', []));
    const [g, setG] = useState<GlobalList[]>(() => s('erp_gl_v14', []));
    useEffect(() => { localStorage.setItem('erp_rec_v14', JSON.stringify(r)); }, [r]);
    useEffect(() => { localStorage.setItem('erp_df_v14', JSON.stringify(d)); }, [d]);
    useEffect(() => { localStorage.setItem('erp_gl_v14', JSON.stringify(g)); }, [g]);
    return (<AuthContext.Provider value={{ records: r, setRecords: setR, dynamicFields: d, setDynamicFields: setD, globalLists: g, setGlobalLists: setG }}><AppInner /></AuthContext.Provider>);
};
